...
const OrderSkhema = Yup.object({
    username: Yup.string().min(2, "Too short").max(20, "Too long").required("Username is required"),
    email: Yup.string().email("Email is not valid").required("Email is required"),
    delivery: Yup.string().oneOf(["pickup", "courier", "drone"], "Delivery is not valid").required("Delivery is required"),
    restrictions: Yup.array().of(
    Yup.string().oneOf(["vegan", "gluten-free", "nut-free"], "Restrictions is not valid")),
    deliveryTime: Yup.string().oneOf(["morning", "afternoon", "evening"], "Delivery time is not valid").required("Delivery time is required"),
message: Yup.string().max(200, "Too large")
...
export default function OrderForm() {
    ...
    return (
        <Formik initialValues={INITIAL_VALUES} validationSchema={ OrderSkhema } onSubmit={handleSubmit}>
